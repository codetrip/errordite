    ...
    public class RedisSession : ComponentBase, IRedisSession
    {
        ...
        public bool TryOpenConnection()
        {
            ...
            lock (_connectionSyncLock)
            {
                ...
                {
                    Error(new ErrorditeRedisConnectivityException(string.Format("Unable to connect to Redis: {0}", se.Message), false, se));
                    _connectionEstablished = false;
                }
                catch (TimeoutException te)
                {
                    Error(new ErrorditeRedisConnectivityException(string.Format("Unable to connect to Redis: {0}", te.Message), false, te));
                    _connectionEstablished = false;
                }
            }
