...
(function ($) {
    ...
    var NATIVE_SUPPORT = !!("placeholder" in document.createElement("input") && !/Apple/.test(navigator.vendor));
    //gt: at the present time (see 4377 / https://bugs.webkit.org/show_bug.cgi?id=51290)
    //safari does not wrap native placeholder text in textareas; hence disable it
...
// TODO: maybe style special characters inside a regexp as punctuation.
  ...
  function prettyPrint(opt_whenDone) {
    ...
    //GT: this method used to be called now.  Stupid monkey patching.  See http://stackoverflow.com/questions/11376808/why-does-google-code-prettifier-stop-working-when-i-use-jquery-datepicker-or-vi
    var clock = Date;
    if (!clock['nowInt']) {
      clock = { 'nowInt': function () { return +(new Date); } };
    }
    ...
    function doWork() {
      var endTime = (window['PR_SHOULD_USE_CONTINUATION'] ?
                     clock['nowInt']() + 250 /* ms */ :
                     Infinity);
      for (; k < elements.length && clock['nowInt']() < endTime; k++) {
        var cs = elements[k];
        ...
        var DRPTemplate = '<div class="daterangepicker dropdown-menu">' +
                ...
                '<div class="ranges">' +
                  '<div class="range_inputs">' +
                    ...
                    '</div>' +
                    '<button class="btn btn-small btn-blue" disabled="disabled" style="margin-top:5px;">' + this.locale.applyLabel + '</button>' +
                  '</div>' +
                '</div>' +
              '</div>';
